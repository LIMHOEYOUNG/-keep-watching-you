...
static char sqlAddr[]= "20.39.200.157";
char sqlId[]= "jetson"; 
char sqlPasswd[]= "qwer1234";
char driveUserId[50];
//char jwt[];
...
int main() {
  ...
  signal(SIGINT, OnSignal); // 인터럽트 시그널 콜백 설정
	...
	strcpy(fileName, ymd);
	strcat(fileName, hms);
  sprintf(sqlfileName,"%s",fileName);
	strcat(fileName, ".json");

  sprintf(filePath,"/home/driveuser/%s/drivelog/%s.JSON",sqlId,fileName);
  //SQL설정
  ...
  if (mysql_real_connect(conn, sqlAddr, sqlId, sqlPasswd, "TEST", 0, NULL, 0) == NULL) {
...
void inputSQL(){
  ...
  if (mysql_query(conn, sqlorder)) {
        fprintf(stderr, "mysql_query failed(sqlInsert()): %s\n", mysql_error(conn));
    }
...
void sqldrivelog(){
  ...
  sprintf(sqlorder, 
    "INSERT INTO userdrivelog(id, userid, start_time, start_location, end_time, end_location, drive_log) VALUES ('%s','%s','%s','%s','%s','%s','%s')"
    ,sqlfileName,sqlId,drive_startTime,startloc,drive_lastTime,lastloc,filePath);
  
  inputSQL();
}
void sqlgps(char flag){
  if(flag==1){
    sprintf(sqlorder,"UPDATE TEST.gps SET gpslocation = '%s' WHERE (id = '%s'and userid = '%s')",lastloc,sqlfileName,sqlId);
    inputSQL();
  }
  else{
    sprintf(sqlorder, "INSERT INTO gps(id, userid, gpslocation) VALUES ('%s','%s','%s')",sqlfileName,sqlId,lastloc);
    inputSQL();
  }
}  