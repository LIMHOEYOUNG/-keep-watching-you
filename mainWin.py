...
def login():
    try:
        status_check=loginWin.login_run(user_id,password)
        #if(status_check):
...
tk1.Button(window, text = "Face", command = face_rec).grid(row = 1, column = 0, padx = 30, pady = 15)
tk1.Button(window, text = "Login", command = login).grid(row = 1, column = 1, padx = 30, pady = 15)
tk1.Label(window, text = "test").grid(row = 2, column = 0, columnspan=2)
